...
bool is_fibonacci(int *ptr, int len, int stride) {
    ASSERT(len >= 3, "`len` should be at least 3");   //ASSERT 一般不是普通的流程控制。如果条件可能正常地不成立，通常应使用 if；ASSERT 更适合表示“按照程序设计，这个条件必须成立，否则说明调用方式或程序存在错误”。
...
int main(int argc, char **argv) {
    ...
    // clang-format off
    ASSERT( is_fibonacci(arr0    , sizeof(arr0) / sizeof(*arr0)    , 1),         "arr0 is Fibonacci"    );   //*arr0 表示 arr0 的第一个元素
